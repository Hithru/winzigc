...
//        System.out.println(nextToken.type);
    ...
    void CaseExpression(ASTNode parent){
        ...
        if(nextToken.type == ".."){
            ...
            read("..");
            doubleDot.addChildAtIndex(0, parent.removeChildNode(parent.getChildNodes().size()-2));
        ...
        if(nextToken.type == "<=" || nextToken.type == "<" || nextToken.type == ">="|| nextToken.type == ">"|| nextToken.type == "="|| nextToken.type == "<>"){
            switch(nextToken.type){
                case "<=":
                    ...
                    lessOrEqNode.addChildAtIndex(0, parent.removeChildNode(parent.getChildNodes().size()-2));
                    Term(lessOrEqNode);
                    break;
                case "<":
                    read("<");
                    ASTNode lessNode = addASTNode(parent, "<");
                    lessNode.addChildAtIndex(0, parent.removeChildNode(parent.getChildNodes().size()-2));
                    Term(lessNode);
                    break;
                case ">=":
                    read(">=");
                    ASTNode greaterOrEqNode = addASTNode(parent, ">=");
                    greaterOrEqNode.addChildAtIndex(0, parent.removeChildNode(parent.getChildNodes().size()-2));
                    Term(greaterOrEqNode);
                    break;
                case ">":
                    read(">");
                    ASTNode greaterNode = addASTNode(parent, ">");
                    greaterNode.addChildAtIndex(0, parent.removeChildNode(parent.getChildNodes().size()-2));
                    Term(greaterNode);
                    break;
                case "=":
                    read("=");
                    ASTNode equalNode = addASTNode(parent, "=");
                    equalNode.addChildAtIndex(0, parent.removeChildNode(parent.getChildNodes().size()-2));
                    Term(equalNode);
                    break;
                case "<>":
                    read("<>");
                    ASTNode InequalNode = addASTNode(parent, "<>");
                    InequalNode.addChildAtIndex(0, parent.removeChildNode(parent.getChildNodes().size()-2));
                    Term(InequalNode);
                    break;
                default:
        ...
        while (nextToken.type == "+" || nextToken.type == "-" || nextToken.type == "or") {
            switch (nextToken.type) {
                case "+":
                    ...
                    addNode.addChildAtIndex(0, parent.removeChildNode(parent.getChildNodes().size()-2));
                    Factor(addNode);
                    break;
                case "-":
                    read("-");
                    ASTNode minusNode = addASTNode(parent, "-");
                    minusNode.addChildAtIndex(0, parent.removeChildNode(parent.getChildNodes().size()-2));
                    Factor(minusNode);
                    break;
                case "or":
                    read("or");
                    ASTNode orNode = addASTNode(parent, "or");
                    orNode.addChildAtIndex(0, parent.removeChildNode(parent.getChildNodes().size()-2));
                    Factor(orNode);
                    break;
                default:
        ...
        while(nextToken.type == "*" || nextToken.type == "/" ||nextToken.type == "and" ||nextToken.type == "mod" ){
            switch(nextToken.type){
                case "*":
                    ...
                    mulNode.addChildAtIndex(0, parent.removeChildNode(parent.getChildNodes().size()-2));
                    Factor(mulNode);
                    break;
                case "/":
                    read("/");
                    ASTNode divNode = addASTNode(parent, "/");
                    divNode.addChildAtIndex(0, parent.removeChildNode(parent.getChildNodes().size()-2));
                    Factor(divNode);
                    break;
                case "and":
                    read("and");
                    ASTNode andNode = addASTNode(parent, "and");
                    andNode.addChildAtIndex(0, parent.removeChildNode(parent.getChildNodes().size()-2));
                    Factor(andNode);
                    break;
                case "mod":
                    read("mod");
                    ASTNode modNode = addASTNode(parent, "mod");
                    modNode.addChildAtIndex(0, parent.removeChildNode(parent.getChildNodes().size()-2));
                    Factor(modNode);
                    break;
            }
        }
